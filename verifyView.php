...
<body>
    ...
    <?php
        ...
        session_start();
        $commentslist = $mysqli->prepare("SELECT comments.content FROM comments WHERE comments.story_id = ?");
        if(!$commentslist)
        {
        ...
        else
        {
            ...
            $commentslist->bind_result($contentlist);
            $commentcount == 0;
            while($commentslist->fetch())
            {
                printf("<br /> Comment: %s <br />", htmlspecialchars($contentlist));
